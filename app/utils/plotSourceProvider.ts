import {CognitiveLoadData, DataSources, VisualAttentionDataSource} from '@/types';
  ...
  for (const key in simulationDataSet){
    for (const innerKey in simulationDataSet[key]) {
      if (innerKey == 'video' || innerKey == 'actions') {
        ...
        const simData = simulationDataSet[key][innerKey as 'cognitiveLoad' | 'visualAttention'] as CognitiveLoadData | VisualAttentionDataSource;
...
export const fetchSourceFilesIndex = async (): Promise<DataSources> => {
  const response = await fetch(sourceFiles);
  const dataSources = await response.json();
  return prependBaseUrl(dataSources, baseUrl);
};

const PlotsFileSource: DataSources = {
  '09182024': {
    video: {
      name: 'Video',
